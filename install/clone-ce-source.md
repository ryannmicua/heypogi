...
2. Keeps the local source checkout available for the skills installer at `install/install-ce-skills.md`
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File install/scripts/clone-ce-source.ps1
```

To suppress prompts (e.g. for scripting):

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File install/scripts/clone-ce-source.ps1 -Quiet
```
