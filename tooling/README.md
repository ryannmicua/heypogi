...
| `~/.paseo/config.json` (Linux) | `dev-stack.sh install -a paseo` additive seed/merge from `dotfiles/paseo/config.json` | `daemon.listen: "0.0.0.0:6767"`, `features.webUi.enabled: true`, `daemon.auth.password` (bcrypt, preserved never overwritten); the user unit runs `paseo daemon run` (foreground) and reads listen/web UI from this config |
