...
function Get-RepoRootFromScriptLocation {
  ...
  # This script lives at: <repo>/tooling/<domain> (two levels below repo root)
...
$destPath = Join-Path $destDir "heypogi"
...
$expected = (Resolve-Path -LiteralPath $skillsRoot).Path
$installed = 0
$skipped = 0
