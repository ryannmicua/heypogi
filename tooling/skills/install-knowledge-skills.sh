#!/usr/bin/env bash
#=======================================================================
# Script:    install-knowledge-skills.sh
# Purpose:   Reconciler linking the Compound Knowledge plugin skills (external checkout) into ~/.agents/skills/compound-knowledge.
...
# Usage:     install-knowledge-skills.sh [status|install] [--create-dest] [-f|--force]
#                                  [-q|--quiet] [--dry-run] [-h|--help]
#
# Managed state: ~/.agents/skills/compound-knowledge symlink -> external/compound-knowledge/plugins/compound-knowledge/skills.
...
usage() {
  cat <<'EOF'
Install Compound Knowledge plugin skills by linking the external checkout into ~/.agents/skills/compound-knowledge.

Usage:
  bash tooling/skills/install-knowledge-skills.sh [status] [-q|--quiet] [-h|--help]
  bash tooling/skills/install-knowledge-skills.sh install [--create-dest] [-f|--force] [-q|--quiet] [--dry-run] [-h|--help]
...
skills_root="${repo_root}/external/compound-knowledge/plugins/compound-knowledge/skills"
...
if [[ ! -d "${skills_root}" ]]; then
  ...
  log_err "Cannot install compound-knowledge skills without sources."
  exit 1
fi
...
do_status() {
  ...
  st="$(link_state)"
  case "$st" in
    ok) log_ok "compound-knowledge converged (${dest_path} -> ${expected_target})"; return 0 ;;
    missing) log_warn "compound-knowledge link missing: ${dest_path}"; return 1 ;;
    wrong-link) log_warn "compound-knowledge link points elsewhere: ${dest_path} -> $(readlink "${dest_path}") (want ${expected_target})"; return 1 ;;
    conflict-file|conflict-dir) log_warn "compound-knowledge destination is a conflicting ${st#conflict-}: ${dest_path}"; return 1 ;;
  esac
}
...
do_install() {
  ...
  case "$st" in
    ok)
      log_ok "compound-knowledge already converged; nothing to do."
      printf 'Installed: 0\nSkipped:   1\n'
      return 0
      ;;
    missing)
