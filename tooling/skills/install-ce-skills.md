...
Explain why: Codex and OpenCode discover user skills from `~/.agents/skills`, so the folder is needed as the destination for the link that makes the CE plugin skills available without copying.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tooling/skills/install-ce-skills.ps1
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File tooling/skills/install-ce-skills.ps1 -CreateDest
```
...
```bash
bash tooling/skills/install-ce-skills.sh
```
...
- The installer uses one link for the whole CE skills folder, not one link per skill.
...
- If `~/.agents/skills/compound-engineering` already exists, the scripts stop and show the existing target versus the intended target before asking whether to overwrite, skip, or quit.
