...
# Purpose:   Reconciler linking this repo's skills (src/skills) into ~/.agents/skills/heypogi.
...
# Usage:     install-skills.sh [status|install] [--create-dest] [-f|--force]
#                                  [-q|--quiet] [--dry-run] [-h|--help]
#
# Managed state: ~/.agents/skills/heypogi symlink -> src/skills.
...
Usage:
  bash tooling/skills/install-skills.sh [status] [-q|--quiet] [-h|--help]
  bash tooling/skills/install-skills.sh install [--create-dest] [-f|--force] [-q|--quiet] [--dry-run] [-h|--help]
...
if [[ ! -d "${skills_root}" ]]; then
  ...
  log_err "Cannot install heypogi skills without sources."
  exit 1
fi
...
do_status() {
  ...
  st="$(link_state)"
  case "$st" in
    ok) log_ok "heypogi converged (${dest_path} -> ${expected_target})"; return 0 ;;
    missing) log_warn "heypogi link missing: ${dest_path}"; return 1 ;;
    wrong-link) log_warn "heypogi link points elsewhere: ${dest_path} -> $(readlink "${dest_path}") (want ${expected_target})"; return 1 ;;
    conflict-file|conflict-dir) log_warn "heypogi destination is a conflicting ${st#conflict-}: ${dest_path}"; return 1 ;;
  esac
}
...
do_install() {
  ...
  case "$st" in
    ok)
      log_ok "heypogi already converged; nothing to do."
      printf 'Installed: 0\nSkipped:   1\n'
      return 0
      ;;
    missing)
