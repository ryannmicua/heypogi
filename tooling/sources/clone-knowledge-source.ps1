...
if ($Help) {
  Write-Host "clone-knowledge-source.ps1 - clone/update the Compound Knowledge plugin source" -ForegroundColor Green
  Write-Host ""
  Write-Host "Usage: clone-knowledge-source.ps1 [-Quiet]"
  Write-Host ""
  Write-Host "Clones external/compound-knowledge if missing, or pulls latest if it"
...
$targetDir = Join-Path $externalDir "compound-knowledge"
$cloneUrl = "https://github.com/EveryInc/compound-knowledge-plugin.git"
...
if (Test-Path -LiteralPath $targetDir -PathType Container) {
  ...
  if ($Quiet) {
    ...
    Write-Host "Compound Knowledge source already cloned at: $targetDir" -ForegroundColor Green
  ...
  Write-Host "Cloning compound-knowledge source into $targetDir ..." -ForegroundColor Cyan
...
if ($didUpdate) {
  & $statusRecorder -Name "compound-knowledge" -RepositoryPath $targetDir
}

if (-not $Quiet) {
  Write-Host ""
  Write-Host "Compound Knowledge source: $targetDir" -ForegroundColor Green
