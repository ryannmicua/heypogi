...
# Network: https://github.com/EveryInc/compound-engineering-plugin.git (clone/pull only; HTTPS, finite timeouts).
...
usage() {
  cat <<'EOF'
Clones the Compound Engineering source repository into external/compound-engineering/.

Usage:
  bash tooling/sources/clone-ce-source.sh [-f|--force] [-q|--quiet] [--dry-run] [-h|--help]
...
target_dir="${external_dir}/compound-engineering"
clone_url="https://github.com/EveryInc/compound-engineering-plugin.git"
clone_args="--depth 1"
...
if [[ -d "${target_dir}" ]]; then
  ...
  if [[ "$FORCE" == true ]]; then
    ...
    log_info "Compound Engineering source already cloned at: ${target_dir}"
    printf "Pull latest? [y/N] " >&2
    IFS= read -r answer || answer=""
    answer="$(printf "%s" "${answer}" | tr '[:upper:]' '[:lower:]' | xargs || true)"
    [[ "${answer}" == "y" ]] && pull=true
  else
    log_info "Compound Engineering source present at ${target_dir}; skipping pull (non-interactive without -f)."
  fi
  if [[ "$pull" == true ]]; then
    ...
    if ! git_net -C "${target_dir}" pull; then
      ...
      log_err "git pull failed for Compound Engineering."
      exit 1
    fi
    did_update=1
  fi
  ...
  if ! git_net clone --depth 1 "${clone_url}" "${target_dir}"; then
    ...
    log_err "git clone failed for Compound Engineering."
    exit 1
  fi
  did_update=1
fi
...
if [[ "${did_update}" -eq 1 ]]; then
  if [[ "$QUIET" == true ]]; then
    bash "${status_recorder}" --name compound-engineering --repository-path "${target_dir}" -q
  else
    bash "${status_recorder}" --name compound-engineering --repository-path "${target_dir}"
  fi
fi

if [[ "$QUIET" != true ]]; then
  printf "\n"
  printf "Compound Engineering source: %s\n" "${target_dir}"
  printf "  Branch: "
...
log_ok "Compound Engineering source ensured."
