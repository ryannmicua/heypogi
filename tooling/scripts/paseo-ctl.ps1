...
$ValidCommands = @("install", "update", "status", "start", "stop", "uninstall", "help")
...
function Invoke-Start {
  if (Get-ListeningPid -Port $PaseoPort) {
...
function Invoke-Status {
  ...
  $daemonPid = Get-ListeningPid -Port $PaseoPort
  Write-Host "  Daemon running  : $(if ($daemonPid) { "YES (pid $daemonPid, port $PaseoPort)" } else { 'no' })"

  $task = Get-ScheduledTask -TaskName $PaseoTaskName -ErrorAction SilentlyContinue
  Write-Host "  Autostart task  : $(if ($task) { "$PaseoTaskName ($($task.State))" } else { 'not registered' })"

  $cfgPath = Join-Path $env:USERPROFILE ".paseo\config.json"
  if (Test-Path -LiteralPath $cfgPath) {
    try {
      $cfg = Get-Content -LiteralPath $cfgPath -Raw | ConvertFrom-Json
      Write-Host "  Config listen   : $($cfg.daemon.listen)"
      Write-Host "  Web UI enabled  : $($cfg.features.webUi.enabled)"
      Write-Host "  Password set    : $(if ([string]::IsNullOrEmpty("$($cfg.daemon.auth.password)")) { 'no' } else { 'yes' })"
    } catch {
      Write-Host "  Config          : $cfgPath (could not parse)" -ForegroundColor Yellow
    }
  } else {
    Write-Host "  Config          : not found ($cfgPath)"
  }
}
...
function Invoke-Uninstall {
  if (Get-ListeningPid -Port $PaseoPort) {
    if (-not $Force) {
      Write-Host "The Paseo daemon is currently running. This will stop it and any running agents." -ForegroundColor Yellow
  ...
  if ($task) {
    try {
      ...
      Write-Host "Unregistered $PaseoTaskName scheduled task." -ForegroundColor Cyan
    } catch {
      Write-Host "Could not unregister ${PaseoTaskName}: $($_.Exception.Message) (try an elevated shell)" -ForegroundColor Red
    }
  }

  if ($WipeConfig) {
    if (-not $Force) {
      Write-Host "This will also delete ~/.paseo (config, including any provider API keys) and Paseo desktop's client token for it." -ForegroundColor Yellow
      $choice = Read-Choice -Prompt "Are you sure? [Y]es, [N]o?" -ValidChoices @("Y", "N")
      if ($choice -eq "N") { exit 0 }
    }
    $cfgDir = Join-Path $env:USERPROFILE ".paseo"
  ...
  npm uninstall -g @getpaseo/cli 2>&1 | Out-Null
  Write-Host "Uninstalled npm package @getpaseo/cli." -ForegroundColor Cyan
  Write-Host "Paseo cleanup complete." -ForegroundColor Green
}

# ---------- help ----------
function Invoke-Help {
  Write-Host "paseo-ctl.ps1 - manage the Paseo CLI/daemon install" -ForegroundColor Green
  Write-Host ""
  Write-Host "Usage: paseo-ctl.ps1 [command] [-Quiet] [-Force] [-WipeConfig]"
  Write-Host ""
  Write-Host "Commands:"
  Write-Host "  install    Install/update the @getpaseo/cli npm package (newest of latest/beta)."
  Write-Host "  update     Alias for install."
  Write-Host "  status     Show install/daemon/autostart/config status. (default when no command given)"
  Write-Host "  start      Start the daemon (no-op if already running)."
  Write-Host "  stop       Stop the daemon."
  Write-Host "  uninstall  Stop the daemon, unregister the scheduled task, and uninstall the npm package."
  Write-Host "  help       Show this message."
  Write-Host ""
  Write-Host "Flags:"
  Write-Host "  -Quiet       Suppress non-essential output / prompts."
  Write-Host "  -Force       Skip interactive confirmations."
  Write-Host "  -WipeConfig  On uninstall, also delete ~/.paseo (config, incl. provider API keys)."
  Write-Host "               Off by default; always confirms unless -Force is also given."
  Write-Host ""
  Write-Host "Registering/unregistering the PaseoDaemon scheduled task requires an elevated"
  Write-Host "shell (it runs with RunLevel Highest)."
}
