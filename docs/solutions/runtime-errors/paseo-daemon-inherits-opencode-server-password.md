...
After — the helper in `tooling/dev-stack/dev-stack.ps1`:
...
- [`tooling/dev-stack/paseo-headless-setup.md`](../../../tooling/dev-stack/paseo-headless-setup.md) — documents starting the daemon directly and registering the `PaseoDaemon` scheduled task, with no mention that the invoking shell's environment is captured permanently. Refresh candidate.
- [`tooling/dev-stack/dev-stack.md`](../../../tooling/dev-stack/dev-stack.md) — reference for the supervisor's `start` / `install` / `fix` commands, which are the code paths patched here. Does not yet document the env-stripping behavior. Refresh candidate.
