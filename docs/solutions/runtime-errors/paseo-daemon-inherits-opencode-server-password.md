...
After — the helper in `install/scripts/check-dev-stack.ps1`:
...
- [`install/check-dev-stack.md`](../../../install/check-dev-stack.md) — reference for the supervisor's `start` / `install` / `fix` commands, which are the code paths patched here. Does not yet document the env-stripping behavior. Refresh candidate.
